...
- **Operator-authored:** generated by the browser dashboard. Examples: terminal command text, screenshot requests, and file operations.
...
1. Operator actions that affect agent state (terminal commands, screenshot requests, session deletion, and file operations) must be logged with operator identity, agent identity, trace ID, and timestamp.
...
1. Add metrics for heartbeat rate, command queue depth, NATS publish latency, screenshot processing duration, and dashboard viewer count.
