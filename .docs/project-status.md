...
| Cryptographic service | Software-provider validation, fail-closed readiness, opaque key handles, command-key separation, and cryptographic consistency probes exist. | The key-generation plan explicitly records partial and missing production controls. |
| CI | Go formatting, tests, race, vet, static/security analysis, vulnerability scan, lint, cross-build, and website format/lint/type/build gates are configured. The website gate and Go cross-build pass locally. | `make ci-go` currently fails on 35 repository-owned golangci-lint findings. Platform integration, browser tests, signing, provenance, SBOM, and release publication do not exist. |
...
2. **Broker protection:** use independently issued NATS mutual-TLS credentials, authenticate and authorize the gateway subject namespace, await durable publish acknowledgement, bound reconnect/publish behavior, and document broker recovery.
...
5. **Cryptographic production gate:** resolve every applicable partial/missing control in `gateway-cryptographic-key-generation-plan.md`, add key-service tests and failure injection, select the actual HSM/KMS or explicitly approved provider and operating environment, implement rotation/recovery/audit controls, and produce independent approval evidence. Disabled future encryption algorithms need not be exposed merely to satisfy the milestone; no unimplemented algorithm may carry a production claim.
...
10. **Existing quality debt:** resolve the current 35 golangci-lint findings without suppressing mandatory rules, add tests for the untested key-service and broker packages, and keep all Go gates green from a clean checkout.
