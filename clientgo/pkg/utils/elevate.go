...
func RerunAsAdmin() {
	...
	if len(os.Args) > 1 && os.Args[1] == "-elevated" {
		logger.GetLogger().Info("Already running with elevated privileges.")
		return
	}

	exePath, err := os.Executable()
	if err != nil {
		logger.GetLogger().Error("Failed to get executable path", zap.Error(err))
	}
	...
	if runtime.GOOS == "windows" {
		...
		if os.Getenv("XENOMORPH_DEV") == "1" {
			...
			err := cmd.Run()
			if err != nil {
				logger.GetLogger().Error("Failed to re-run with elevated privileges", zap.Error(err))
			}
			os.Exit(0)
			return
		}
		...
		if errno, ok := err.(syscall.Errno); ok && errno == 1223 {
			logger.GetLogger().Info("User canceled elevation prompt.")
			os.Exit(0)
		} else if err != nil {
			logger.GetLogger().Error("Failed to re-run with elevated privileges", zap.Error(err))
		}
