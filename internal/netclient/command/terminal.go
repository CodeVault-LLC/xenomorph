...
func initTerminalCommand() {
	err := commandHandler.AddCommand("terminal", func(client types.ClientController, msgID uint32, command types.Command) {
		...
		client.Send(types.MsgCommandResponse, 0, msgID, []byte(resp.ToJSON()))
	})
