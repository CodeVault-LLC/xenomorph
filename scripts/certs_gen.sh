...
openssl req -new -nodes -out certs/client.csr -keyout certs/client.key -subj "/CN=${CLIENT_HOSTNAME}/OU=${AGENT_UUID}"
openssl x509 -req -in certs/client.csr -CA certs/ca.crt -CAkey certs/ca.key -CAcreateserial -out certs/client.crt -days 365 -extfile <(printf "subjectAltName=DNS:%s,URI:urn:xenomorph:agent:%s" "${CLIENT_HOSTNAME}" "${AGENT_UUID}")
